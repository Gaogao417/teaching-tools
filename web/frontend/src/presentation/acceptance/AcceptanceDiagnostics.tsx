 ...
 *
 * 只暴露路由/会话/revision/fallback 状态等事实——不含 hidden canonical
 * nodes、reasoning hypothesis 或任何答案真值（字段白名单）。它不是生产
 * 学生功能：无 acceptance 查询参数时整个组件不渲染。
 *
 * VS1（mvp/vs-01 实施顺序 §6.5）：增加统一 Workspace View 的 revision
 * （`workspace_view.revision`；REQ-05 断言与画布/板书 surface 同 revision）。
 */
...
export interface AcceptanceDiagnosticsProps {
  ...
  sessionId?: string;
  viewRevision?: number;
  /** VS1：统一 StudentWorkspaceView 的 revision（会话 revision 一致性断言）。 */
...
export function AcceptanceDiagnostics({ taskId, route, sessionId, viewRevision, workspaceRevision, fallbackOccurred }: AcceptanceDiagnosticsProps) {
  return (
    <aside
    ...
    >
      ...
      <dl>
        ...
        <div><dt>workspaceRevision</dt><dd>{workspaceRevision ?? "—"}</dd></div>
