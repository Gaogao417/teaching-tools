...
export const enterTextDefinition = createFormMachineDefinition<EnterTextAction>("enter-text", {
  ...
  boardPreview: (context) => {
    const accepted = context.contract.input.expectedValues?.includes(context.answers.value || "");
    return projectBoardSlotValues(context.contract, {
      value: accepted ? context.contract.input.expectedValues?.[0] : context.answers.value,
    });
  },
});
