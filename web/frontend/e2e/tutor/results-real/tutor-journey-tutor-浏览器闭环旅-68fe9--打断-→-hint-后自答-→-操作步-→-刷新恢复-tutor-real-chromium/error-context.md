...
- Name: tutor-journey.spec.ts >> tutor 浏览器闭环旅程 >> 进入 → 回答推进 → 提问 → 打断 → hint 后自答 → 操作步 → 刷新恢复
- Location: e2e/tutor/specs/tutor-journey.spec.ts:22:3
...
Received string:    "连接恢复中"
Timeout: 100000ms

Call log:
  - Expect "toContainText" with timeout 100000ms
  - waiting for getByTestId('tutor-state')
    8 × locator resolved to <span data-testid="tutor-state">老师思考中…</span>
      - unexpected value "老师思考中…"
    194 × locator resolved to <span data-testid="tutor-state">连接恢复中</span>
        - unexpected value "连接恢复中"
