/**
 * Phase 5 remediation 智能链测试（Vitest 迁移版；与 node 版同用例集）：
...
async function testDeepSeekAdapter(): Promise<void> {
  ...
  const outcome = await model.complete<{ ok: boolean }>({ systemPrompt: "s", promptVersion: "p", userPayload: { a: 1 }, timeoutMs: 500 });
  ...
  expect((capturedBody?.messages as Array<{ role: string }>)[1].role).toBe("user");
  expect(outcome.usage?.inputTokens).toBe(5);

  // <think> 剥离 + 首尾大括号截取
  expect(parseModelJson('<think>hidden</think>{"x": 1}')).toEqual({ x: 1 });
  expect(() => parseModelJson("不是 JSON")).toThrowError(/不是合法 JSON/);

  // 未配置 key
  const unconfigured = new DeepSeekStructuredModel({ apiKey: "", fetchImpl: async () => new Response("{}", { status: 200 }) });
  await expect(
    unconfigured.complete({ systemPrompt: "s", promptVersion: "p", userPayload: {}, timeoutMs: 100 }),
  ...
  await expect(
    unauthorized.complete({ systemPrompt: "s", promptVersion: "p", userPayload: {}, timeoutMs: 100 }),
  ).rejects.toMatchObject({ code: "auth-error", retryable: false });
}
...
describe("tutorIntelligence（adapter/gating/projection/validation/graph/adjudication）", () => {
  it("DeepSeek adapter：请求体/错误映射/think 剥离", async () => {
    await testDeepSeekAdapter();
  });
  it("置信度门 / grounding / 暂定投影", () => {
    testGating();
  });
  it("提案校验（文案来源/泄题/档位/证据）", () => {
    testValidation();
  });
  it("图 happy path", async () => {
    await testGraphHappyPath();
  });
  it("图校验失败重试一次", async () => {
    await testGraphRetryOnce();
  });
  it("图两次失败 Wait fallback", async () => {
    await testGraphFallbackAfterTwoFailures();
  });
  it("确定性路径（silence/pointing/action evidence 不调对齐模型）", async () => {
    await testGraphDeterministicPaths();
  });
  it("模型错误转 PolicyFailure", async () => {
    await testGraphModelError();
  });
  it("模型可见目录消毒（裁定 §3）", async () => {
    await testModelCatalogSanitization();
  });
  it("workspace kind 兼容（explain 拒 / prompt 许）", () => {
