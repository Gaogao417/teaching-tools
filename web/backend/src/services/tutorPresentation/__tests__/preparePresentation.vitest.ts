/**
 * P5-09/10：Presenter 与 WorkspaceAction 安全 adapter 测试——一 Move 零到多动作、Wait 零呈现、Confirm 只说话、workspace 五重校验、truth isolation。
 * （Vitest 迁移版；与 node 版同用例集——assert 语义不变。）
 */
import assert from "node:assert/strict";
import { describe, it } from "vitest";

import { preparePresentation, resolveWorkspacePresentation } from "../PreparePresentation";
import { VOICE_SCAFFOLDS } from "../VoiceAction";
...
describe('preparePresentation + workspaceActionAdapter', () => {
  it("node 版全部断言（顺序流）", () => {
