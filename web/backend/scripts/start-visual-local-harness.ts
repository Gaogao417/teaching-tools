...
async function main(){
 ...
 const gate={name:'visual-local-scripted',async adjudicate(contextJson:string){const context=JSON.parse(contextJson);if(context.student_input?.text==='我没听懂，为什么这两组边是对应边？')return JSON.stringify({response_kind:'help_request',verdict:'not_applicable',reasoning_location:'unknown',grounding_refs:[]});return JSON.stringify({response_kind:'understanding_confirmation',matched_gate_id:context.eligible_gates[0]?.gate_id,verdict:'pass',reasoning_location:'unknown',grounding_refs:[]});}};
