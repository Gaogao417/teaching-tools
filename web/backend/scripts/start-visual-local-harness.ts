...
async function main(){
 ...
 const backendPort=Number(arg('--backend-port','3134')),frontendPort=Number(arg('--frontend-port','5194'));
 for(const port of [backendPort,frontendPort])if(!Number.isInteger(port)||port<1024||port>65535)throw new Error('invalid local port');
 const frontendUrl=`http://127.0.0.1:${frontendPort}`,backendUrl=`http://127.0.0.1:${backendPort}`;
 ...
 process.env.TUTOR_VNEXT_GENERATION='1';process.env.FRONTEND_ORIGIN=frontendUrl;
 const canonicalRoot=resolve(arg('--root','/Users/gaochong/develop/teaching-skills-mvp/artifacts/canonical-authoring'));
 ...
 const metadata={mode:'local-mechanical-harness',model:'scripted',tts:'recorded-silence',status:'Draft',publicationPerformed:false,runDir,url:frontendUrl+'/learn/goldenMinhangFold2020'};
 ...
 const server=await new Promise<import('node:http').Server>((done,reject)=>{const s=app.listen(backendPort,'127.0.0.1',()=>done(s));s.once('error',reject);});
 ...
 const vite=spawn(process.execPath,[resolve('../frontend/node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port',String(frontendPort),'--strictPort'],{cwd:resolve('../frontend'),stdio:'inherit',env:{...process.env,VITE_API_BASE_URL:backendUrl,VITE_TEACH_REVIEW:'1'}});
