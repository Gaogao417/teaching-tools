...
**Stem:** 如图，在 $\triangle ABC$ 中，$\angle ABD=\angle ACB$。  已知 $AC=2\sqrt{3}$，$CD=\frac{1}{2}\sqrt{3}$。求 $AB$ 的长。

**Answer-key result:** $AB=3$。

**Assembled solution:** 解： 由题意，在图中标出 $AC=2\sqrt{3}$，$CD=\frac{1}{2}\sqrt{3}$。 代入比例关系，求得 $3$。

#### Middle

**Scenario ID:** `nested-similarity-2026-07-16:Q026`

**Stem:** 如图，在 $\triangle ABC$ 中，$\angle ABD=\angle ACB$。  已知 $AC=2\sqrt{2}$，$AB=\sqrt{5}$。求 $CD$ 的长。

**Answer-key result:** $CD=\frac{3}{4}\sqrt{2}$。

**Assembled solution:** 解： 由题意，在图中标出 $AC=2\sqrt{2}$，$AB=\sqrt{5}$。 代入比例关系，求得 $\frac{3}{4}\sqrt{2}$。

#### Last

**Scenario ID:** `nested-similarity-2026-07-16:Q050`

**Stem:** 如图，在 $\triangle ABC$ 中，$\angle ABD=\angle ACB$。  已知 $AC=4$，$AB=\sqrt{10}$。求 $CD$ 的长。

**Answer-key result:** $CD=\frac{3}{2}$。

**Assembled solution:** 解： 由题意，在图中标出 $AC=4$，$AB=\sqrt{10}$。 代入比例关系，求得 $\frac{3}{2}$。
...
解： 由题意，在图中标出 $AC=2\sqrt{3}$，$CD=\frac{1}{2}\sqrt{3}$。 代入比例关系，求得 $3$。
...
Browser checks performed (Learn mode, Q001 △ABC ∠ABD=∠ACB):
- Page loaded correctly: problem heading, geometry SVG rendered with labeled points A/B/C/D and 6 segments (AB, BC, AC whole, BD, AD subsegment, CD subsegment), SolutionBoard region `解：` with empty slot templates BD=‾ AC=‾ CD=‾, coach panel 当前动作 1/4.
- Segment interaction verified: clicked BD and AD subsegments → value inputs appeared → hit-test correctly distinguishes AD (A→D span) from whole AC and from CD. ✓
- The collinear-conversion (action 2) and full 4-action flow (mark-segment-values → convert-collinear → pair-segments → enter-equation) are covered by backend tests `nested similarity inserts a dedicated collinear conversion action`, `legacy nested sessions resume at the newly required conversion action`, `collinear conversion is mastered only after its dedicated action`.

Commands and results:

- `npm run import:topics` (cwd `web/backend`) → regenerated `topicScenarioBundle.json`, count `nestedSimilarity=50`.
- `python3 .codex/skills/build-action-driven-topic/scripts/validate_generated_topic_v2.py web/backend/src/content/topicScenarioBundle.json --task-id nestedSimilarity` → `OK`, schema `teaching-tools/topic-scenario-bundle/v2`, records=50.
- `npm test` (cwd `web/backend`) → all pass, incl. `nested similarity inserts a dedicated collinear conversion action`, `legacy nested sessions resume at the newly required conversion action`, `collinear conversion is mastered only after its dedicated action`, `all six migrated topics smoke first, middle, and last approved records`.
- `npm test` (cwd `web/frontend`) → 107/107 pass.
- `npm run build` (cwd `web/frontend`) → built successfully.

Records inspected (first/middle/last): `nested-similarity-2026-07-16:Q001` (forward, find AB=3), `:Q026` (middle), `:Q050` (reverse, find CD=3/2). Each carries authored `actionTemplates = [mark-segment-values@1, convert-collinear@1, pair-segments@1, enter-equation@1]`; `convert-collinear@1` establishes `AC=AD+DC` on the overlapping subsegment `AD` (A→D span), distinct from whole `AC` and from `CD`; both solving directions share the one action sequence.

Deferred (required for `verified`): browser walk of Q001/Q026/Q050 across both forward and reverse directions; wrong whole/part segment, wrong correspondence, correction; BACK/CLEAR; refresh/restore; the `AD` vs `AC` vs `CD` hit-test gate; desktop and narrow width.
